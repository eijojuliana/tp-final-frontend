  ...
  historialFiltrado = computed(() => {
    ...
    if (f) {
      result = result.filter(h =>
        String(h.historialCuentaBancariaId).includes(f) ||
        String(h.cuentaBancariaId).includes(f) ||
    ...
    result = result.slice().sort((a: any, b: any) => {
      let cmp = 0;
      switch (attr) {
        case 'cuentaBancariaId': cmp = (a.cuentaBancariaId ?? 0) - (b.cuentaBancariaId ?? 0); break;
